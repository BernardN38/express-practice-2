...
beforeEach(async function () {
  ...
  for (let user of sampleUsers) {
    ...
    tokens[user[0]] = createToken(user[0], user[5]);
  }
});
...
describe("GET /users", function () {
  ...
  test("should list all users", async function () {
    console.log(tokens)
    const response = await request(app)
      .get("/users").set('authorization', tokens['u1'])
    expect(response.statusCode).toBe(200);
...
describe("GET /users/[username]", function () {
  ...
  test("should return data on u1", async function () {
    const response = await request(app)
      .get("/users/u1").set('authorization', tokens['u1'])
    expect(response.statusCode).toBe(200);
...
describe("PATCH /users/[username]", function () {
  ...
  test("should patch data if admin", async function () {
    const response = await request(app)
    .patch("/users/u1").set('authorization', tokens['u3']).send({ "first_name": "new-fn1" }); // u3 is admin
